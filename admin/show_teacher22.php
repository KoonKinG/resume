...
<body>
                        ...
                        <form action="show_teacher22.php" method="POST" target="iframe2">

                            ชื่ออาจารย์ : <input type="text" name="nameTeacher" value="">
...
majorname <input type="text" name="major_name" value="">
	...
	<input type="submit" name="submit" value="ค้นหา">
                ...
                <b>
                    <div class="card-header text-white bg-dark">ตารางสอน</div>
                </b>
...
?>
      ...
      JOIN major
			...
			Where 
			nameTeacher LIKE '%$nameTeacher%' and day_id=$i;";
		 	...
		 	for($j=0;$j<=12;$j++)
			{
				if($j==0){
		 		...
		 		if($j>=$row['time_s']-1 and $j<=$row['time_x']-1){
                    ...
                    echo $row['subject_name']."-".$row['subjectcode']."-".$row['major_name']."-".$row['room_name'];
                }elseif($j==$row['time_x']){
                    echo "</td>";
                }else{
